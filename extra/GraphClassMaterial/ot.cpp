...
void printAdjList(){
	for (int i = 0; i < n; ++i) // para todos os vértices
	{
		...
		{	if (j < adj_list[i].size()-1)
				printf("(%d,%d) > ", adj_list[i][j].first+1, adj_list[i][j].second);
			else 	printf("(%d,%d)", adj_list[i][j].first+1, adj_list[i][j].second);
		}
		printf("\n");
	}
	printf("\n");
...
int main(int argc, char const *argv[])
{
	...
	while (cin >> u >> v) { // o grafo é direcionado direcionado e arestas nao tem peso!
